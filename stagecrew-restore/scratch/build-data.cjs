...
const CMS_ASSET = (id, q = 'width=1600&format=webp&quality=75') =>
  id ? `https://cms.stagecrew.studio/assets/${id}?${q}` : null;

/** Bunny requires filename with extension (e.g. uuid.jpg) — SOURCE: prod Nuxt img URLs */
const BUNNY = (fileOrId, q = 'width=1600&format=webp&quality=75') => {
  if (!fileOrId) return null;
  if (typeof fileOrId === 'string') {
    const key = fileOrId.includes('.') ? fileOrId : `${fileOrId}.jpg`;
    return `https://stagecrew-media.b-cdn.net/${key}?${q}`;
  }
  const key = fileOrId.filename_disk || `${fileOrId.id}.jpg`;
  return `https://stagecrew-media.b-cdn.net/${key}?${q}`;
};
...
function fileMedia(file) {
  ...
  return {
    ...
    src: isVideo ? file.vimeo_url || CMS_ASSET(id) : BUNNY(file) || CMS_ASSET(id),
    poster: file.vimeo_thumb || (isVideo ? null : BUNNY(file, 'width=800&format=webp&quality=60')),
...
const projects = projectsRaw
  ...
  .map((p) => ({
    ...
    text: p.text || '',
    areas: (p.areas || []).map((a) => a.areas_id?.title).filter(Boolean),
...
const site = {
  name: 'STAGECREW',
  header: { externalLinks: header.external_links || [] },
  footer: {
  ...
  backstage: { title: pageBackstage.title, text: pageBackstage.text },
  projects,
};
...
console.log('ok', projects.length, OUT);
